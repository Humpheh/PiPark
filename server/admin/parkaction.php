...
if (isset($_POST['park_action']) && $_POST['park_action'] == 'add') {
	...
	$keys = array("park_name", "park_description", "park_spaces");
	foreach ($keys as $key)
		if (!array_key_exists($key, $_POST))
			json_error('Incomplete post data.');

	if( is_int ( trim($_POST[ 'park_spaces' ]) ) )
		json_error('Park spaces is not a number. (new)');

	// Register the space on the database.
	$query2 = "INSERT INTO parks (park_name, park_desc, park_spaces, park_curspaces) VALUES (?, ?, ?, ?)";
	...
	$stmt2 -> bindValue(3, intval($_POST["park_spaces"]), PDO::PARAM_INT);
	$stmt2 -> bindValue(4, intval($_POST["park_spaces"]), PDO::PARAM_INT);
	$stmt2 -> execute();
	
} else if (isset($_POST['park_action']) && $_POST['park_action'] == 'update') {
	
	// Checks that all the required keys are present in the post data
	$keys = array("park_id", "park_name", "park_description", "park_spaces");
	foreach ($keys as $key)
		if (!array_key_exists($key, $_POST))
			json_error('Incomplete post data.');

	if( is_int ( trim($_POST[ 'park_spaces' ]) ) && is_int ( trim($_POST[ 'park_id' ]) ) )
		json_error('Park spaces is not a number. (update)');

	

	// Register the space on the database.
	$query2 = "UPDATE parks SET park_name = ?, park_desc = ?, park_spaces = ?, park_curspaces = ? WHERE park_id = ?";
	$stmt2 = DB::get() -> prepare($query2);
	$stmt2 -> bindValue(1, $_POST["park_name"], PDO::PARAM_INT);
	$stmt2 -> bindValue(2, $_POST["park_description"], PDO::PARAM_INT);
	$stmt2 -> bindValue(3, intval($_POST["park_spaces"]), PDO::PARAM_INT);
	$stmt2 -> bindValue(4, intval($_POST["park_spaces"]), PDO::PARAM_INT);
	$stmt2 -> bindValue(5, intval($_POST["park_id"]), PDO::PARAM_INT);
	$stmt2 -> execute();
